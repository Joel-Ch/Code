...
int InputValue(char *prompt)//input function (same as Q2 but with int not float)
{
    // input as a float
    int value;
    while (1)
...
int main()
{
    ...
    if (year >= 1901 && year <= 1924)
        ...
        printf("Generation Alpha");//big if statement to check for all cases
    else
        printf("error: year out of range");//otherwise suitable error message
    return 0;
}
